"""
NTB Seychelles — Expression of Interest Scraper
=================================================
Crawls every pagination page of:
    https://www.ntb.sc/tenders/expression-of-interest?start=X
and for each EOI entry fetches the detail page to extract all available
structured fields. PDFs are optionally downloaded too.

Key insight vs. the other two scrapers
---------------------------------------
EOIs have NO bid amounts — they are pre-tender market soundings or
Limited Bidding Method invitations. All meaningful data (description,
deadline, tags, org) lives on the HTML pages, not inside PDFs.
PDFs (when accessible) contain the full EOI document text, which is
extracted and stored in a `pdf_text` column for downstream NLP / search.

Output columns
--------------
    title               – EOI title
    org                 – Procuring entity (extracted from description or title prefix)
    description         – Full body text from the detail page
    created_date        – e.g. "25 March 2024"
    submission_deadline – e.g. "2024-04-12"
    submission_time     – e.g. "10:00"
    tags                – comma-separated tag labels (e.g. "Good and Services, Vehicles")
    eoi_type            – "EOI" | "Limited Bidding" | "Prequalification" | "Other"
                          (inferred from description text)
    category            – inferred project category (same classifier as other scrapers)
    pdf_url             – URL of attached PDF (empty if none)
    pdf_text            – Extracted PDF text (empty if no PDF or 404)
...
Usage
-----
    # Full scrape — all 28 pages
    python ntb_eoi_scraper.py

    # Test run — first 3 pages only
    python ntb_eoi_scraper.py --max-pages 3

    # Re-parse detail pages already cached in --cache-dir (skips network)
    python ntb_eoi_scraper.py --skip-fetch --cache-dir ./ntb_eoi_cache

    # Also download PDFs (saved to --pdf-dir)
    python ntb_eoi_scraper.py --download-pdfs

    # Custom output
    python ntb_eoi_scraper.py --out eoi.csv
"""
